...
if __name__ == '__main__':
    ...
    parser.add_argument('--logpath', default='./results/cutout') #希望保存的logs路径，包括tensorboard
    parser.add_argument('--model', default='cutout',choices=['baseline','cutmix','cutout','mixup'])
    ...
    if args.test:
        ...
        for epoch in range(start_epoch, start_epoch+args.epoch):
            ...
            save_name = os.path.join(args.logpath, 'cutout_latest_epoch.pth')
            torch.save(state, save_name)
            if acc > best_acc:
                print('Saving best model..')
                save_name = os.path.join(args.logpath, 'cutout_best_model.pth')
                torch.save(state, save_name)
                best_acc = acc
